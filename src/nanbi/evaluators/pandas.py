...
class PandasEvaluator(Evaluator):
  ...
  def window_handler(self, op, pandas_df):
    has_partitions = (not op.partition_by is None) or len(op.partition_by) == 0
    has_ordered_by = (not op.order_by is None) or len(op.order_by) == 0
    ...
    if has_ordered_by:
      ...
      pandas_df = pandas_df.sort_values(by=order_by_names, ascending=col_is_asc, inplace=False)
      original_index_order = pandas_df.index

    partitions = pandas_df.groupby(by=partition_by_names) if has_partitions else [None, pandas_df]
    ...
    for _, partition in partitions:
      ...
      if type(op) == opn.OperationRename:
        eval_windows = [pd.Series(self._eval(op.next, window)).rename(op.new_name) for window in windows]
      else:
  ...
  def group_by_handler(self, op):
    ...
    for col in op.group_cols:
      if type(col.op) == opn.OperationRename and is_full_group:
        ...
        group = grouped.apply(lambda df: self._eval(col.op.next, df)).rename(col.op.new_name)
  ...
  def order_by_handler(self, op):
    ...
    return pandas_df.sort_values(by=col_names, ascending=col_is_asc, inplace=False).reset_index(drop=True)
