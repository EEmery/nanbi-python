...
class NanbiTest:
    ...
    def _node_as_dir(obj):
        ...
        for k, v in vars(obj).items():
            is_nanbi_obj = (
                isinstance(v, Operation) or isinstance(v, Column) or isinstance(v, DataFrame)
            )
    ...
    def assertEquals(result, expected, check_exact=True, atol=0.001):
        ...
        if type(result) != type(expected):
            raise AssertionError(
                "Types are mismatching when they should be the same"
        ...
        if result_dir != expected_dir:
            raise AssertionError(
                "The result and expected values doesn't match: "
                + f" Result: {result_dir}"
